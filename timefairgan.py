...
def timegan (ori_data, parameters):
  ...
  if parameters['command'] == 'with_fairness':
    ...
    G_loss_fairness = parameters['lamda_val'] * tf.abs((tf.reduce_mean(loss_underpriv) - tf.reduce_mean(loss_priv)))
  ...
  G_solver = tf.compat.v1.train.AdamOptimizer().minimize(G_loss, var_list = g_vars + s_vars + r_vars)      
  ...
  for itt in range(iterations):
    ...
    for kk in range(2):
      ...
      if parameters['command'] == 'with_fairness':
        ...
        _, step_g_loss_u, step_g_loss_u_e, step_g_loss_s, step_g_loss_v =  sess.run([G_solver, G_loss_U, G_loss_U_e, G_loss_S, G_loss_V], feed_dict={Z: Z_mb, X: X_mb, T: T_mb})
    # Train embedder        
